...
BEGIN
    ...
    RETURN NEW; -- new키워드로 새로 생성된 row에 접근할수 잇다
END;
$$;

--post_upvotes table에 추가할 trigger
-- 이 table에 새로운 row가 추가될때마다
-- post_upvote에 있는 post_id를 가지고 
-- 그 게시물을 posts table에서 찾아서 그 counter을 증가시키겟다.
